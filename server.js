...
【第二步：生成新試卷】
保持相同的主題範疇和相同的任務模式，但情境、計劃名稱、機構、活動內容和人物姓名必須完全不同。
文體：${genreNames[genre] || genre}

【題目格式要求】
題目須包含：情境背景 → 寫作身份（以[職銜][三字中文姓名]名義）→ 文體及場合 → 任務一 → 任務二 → 字數限制（不多於550字）

【資料設計要求】
- 資料一：官方文件形式（通告／宣傳單張），包含背景說明和活動內容表格（2欄：活動名稱｜內容，2-3行）
- 資料二：學生討論記錄（4則留言）：留言一疑慮（對應活動一）、留言二疑慮（對應活動二）、留言三支持、留言四寫作者宣布撰文
- 資料一的活動細項必須能直接回應資料二的疑慮

【示範文章要求】
- 全文不少於550字，寧可略超600字也不可少於550字
- 必須符合${genreNames[genre] || genre}所有格式要求
- 拓展部分用【拓展】和【/拓展】標記

【內容發展評分準則格式（重要）】
contentDevelopment 的 table 只列出具體細項內容，不在每個細項上標注分數。
分數在前端統一以「7-8分/5-6分/3-4分/1-2分/0分」表格呈現。
每個 item 就是細項名稱（如「說明計劃意義」），description 是具體要求，不帶「【X分】」前綴。
